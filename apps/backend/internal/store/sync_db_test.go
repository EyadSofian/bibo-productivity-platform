...
func (f syncFixture) sync(t *testing.T, act []ActivityRow, ks []KeystrokeRow, br []BrowserRow) {
	t.Helper()
	if _, err := f.store.SyncBatch(f.ctx, f.userID, f.bizID, f.deviceID, nil, nil, nil, act, ks, br, nil); err != nil {
...
func TestSyncBatchKeepsDeviceLabelWhenOmitted(t *testing.T) {
	f := newSyncFixture(t)
	label := "Work laptop"
	if _, err := f.store.SyncBatch(f.ctx, f.userID, f.bizID, f.deviceID, &label, nil, nil, nil, nil, nil, nil); err != nil {
