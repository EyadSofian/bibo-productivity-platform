...
const resources = {
  ...
  ar: { common: arCommon, auth: arAuth, onboarding: arOnboarding, welcome: arWelcome, settings: arSettings, permissions: arPermissions, screens: arScreens, media: arMedia },
};
...
/**
 * Mirror the active locale onto <html dir> and <html lang>. `dir` flips the
 * layout (the stylesheet uses logical properties, so the browser does the
 * mirroring); `lang` drives font selection and screen-reader pronunciation.
 *
 * Exported for tests; called on load and on every language change.
 */
